...
class UserService : UserDetailsService {
    ...
    fun saveUser(user: User): Boolean {
        ...
        user.roles = mutableSetOf(Role(1L, "ROLE_USER"))
    ...
    fun setRole(user: Long, role: String){
        val user = this.findUserById(user)
        val role_ = roleRepository!!.findByName(role)
        user.roles!!.add(role_)
        userRepository!!.save(user)
    }
    fun removeRole(user: Long, role: String){
        val user = this.findUserById(user)
        val role_ = roleRepository!!.findByName(role)
